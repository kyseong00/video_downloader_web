import { db, playlists, downloads, deletedVideos } from "./db";
...
export async function checkUserPlaylists(
  userId: string,
  settings: UserSettings,
  options?: { playlistId?: string; fetchAll?: boolean }
): Promise<{ checked: number; queued: number }> {
  const pls = options?.playlistId
    ? await db.select().from(playlists)
        .where(and(eq(playlists.id, options.playlistId), eq(playlists.userId, userId), eq(playlists.isActive, true)))
    : await db.select().from(playlists)
        .where(and(eq(playlists.userId, userId), eq(playlists.isActive, true)));

  if (pls.length === 0) return { checked: 0, queued: 0 };

  // 중복 체크: PLAYLIST 소스 또는 source NULL(마이그레이션 이전 레코드)
  const existingDownloads = await db.select({ url: downloads.url }).from(downloads)
    .where(and(
      eq(downloads.userId, userId),
      or(eq(downloads.source, "PLAYLIST"), isNull(downloads.source)),
    ));
  ...
  // 재다운로드 방지 — PLAYLIST 소스로 삭제된 video ID는 제외
  const deleted = await db.select({ videoId: deletedVideos.videoId }).from(deletedVideos)
    .where(and(eq(deletedVideos.userId, userId), eq(deletedVideos.source, "PLAYLIST")));
  ...
  // 1단계: 모든 플레이리스트에서 영상 목록 수집 (빠름, flat-playlist)
  const downloadTasks: { id: string; url: string; format: string; quality: string }[] = [];

  for (const pl of pls) {
    try {
      // 신규 플레이리스트(최초 체크 전)는 initialMaxVideos만큼만 — 사이트 응답 보호.
      const isFirstCheck = !pl.firstCheckDone;
      const plMaxVideos = isFirstCheck
        ? (pl.initialMaxVideos ?? 10)
        : (options?.fetchAll ? undefined : 30);

      const videos = await getPlaylistVideos(pl.playlistUrl, cookieContent || undefined, plMaxVideos);
      ...
      for (const video of videos) {
        ...
        await db.insert(downloads).values({
          ...
          format: pl.format,
          quality: pl.quality,
          type: ["mp3", "m4a", "wav", "opus"].includes(pl.format) ? "AUDIO" : "VIDEO",
          status: "PENDING",
          progress: 0,
          source: "PLAYLIST",
          playlistId: pl.id,
        ...
        downloadTasks.push({ id, url: video.url, format: pl.format, quality: pl.quality });
      }
      ...
      console.error(`Playlist check error for ${pl.name}:`, err);
    }
  }
  ...
  if (downloadTasks.length > 0) {
    const tasks = downloadTasks.map(t => () =>
      startPlaylistDownload(t.id, t.url, outputDir, t.format, t.quality, userId, settings)
    );
    runWithConcurrency(tasks, maxConcurrent).catch(err => {
      console.error("[playlist-worker] concurrency error:", err);
    });
  }

  return { checked: pls.length, queued: downloadTasks.length };
}
