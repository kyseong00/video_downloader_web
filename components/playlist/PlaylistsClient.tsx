    ...
    <div className="space-y-5 max-w-3xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">다운로드한 파일을 모아 재생하세요</p>
        </div>
          ...
          <DialogContent>
            ...
            <form onSubmit={e => { e.preventDefault(); createMutation.mutate({ name }); }} className="space-y-4 mt-2">
              <div className="space-y-2">
                <Label>이름</Label>
                <Input placeholder="플레이리스트 이름" value={name} onChange={e => setName(e.target.value)} required />
              </div>
              <div className="flex gap-2">
                <Button type="button" variant="outline" className="flex-1" onClick={() => setOpen(false)}>취소</Button>
                <Button type="submit" className="flex-1 bg-[#598392] hover:bg-[#4a7280] text-white" disabled={createMutation.isPending}>만들기</Button>
              </div>
            </form>
          </DialogContent>
        </Dialog>
      ...
      {playlists.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-lg font-medium">플레이리스트가 없습니다</p>
          <p className="text-sm mt-1">새 플레이리스트를 만들어 파일을 추가하세요</p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {new Date(pl.createdAt).toLocaleDateString("ko-KR")} 생성
                  </p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-[#598392] opacity-0 group-hover:opacity-100" title="재생">
